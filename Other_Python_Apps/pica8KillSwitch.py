### Pica8 Cyber KillSwitch Example
...
image3 = './CYBER_WORD.png'
image4 = './killswitch.png'
st.set_page_config(page_title="Pica8 Cyber Kill Switch Portal - SE LAB", page_icon=image1, layout="wide")
...
caption = "AmpCon Login: Cyber Kill Switch Portal Needs to login into AmpCon. Please Enter Your AmpCon Credentials."
col2.write("******************************************")
col1.image(image1, caption=None, width=None, use_column_width=True,
               clamp=False, channels='RGB', output_format='auto')
col1.image(image3, caption=None, width=None, use_column_width=True,
               clamp=False, channels='RGB', output_format='auto')
col2.write("******************************************")

col2.image(image4, caption=None, width=None, use_column_width=True,
               clamp=False, channels='RGB', output_format='auto')
col2.write("***************************************************************")

col1.subheader("Pica8 Cyber Kill Switch")
col1.subheader("Keeping the Network Secure.............")
col1.subheader(".............with the Click of a Button.....")
col1.write("***************************************************************")
     
st.sidebar.image(image1, caption=caption, width=250, use_column_width=False,
                 clamp=False, channels='RGB', output_format='auto')
st.sidebar.title("Pica8 AmpCon Authentication\n & Secure API Key Generator for Cyber Kill Switch Activation")
...
# restartlabmanagerbutton = col2.button('RESTART Cyber Kill Switch Portal')
# col2.write ('PRESS RESTART BUTTON IF \"switchconfigs\" error has occurred (Cyber Kill Switch Portal may periodically lose connection to AmpCon and need token refresh)')
# if restartlabmanagerbutton:
#     killswitch_restart_command = "systemctl restart pica8killswitch"
#     subprocess.call(['bash', '-c', killswitch_restart_command])
...
def LOGIN(username, password, loggedin, pica8apikey, instantlockbutton, backupthelabbutton, goldenlabbackupbutton):
    payload = json.dumps({
        "username": username,
        "password": password
    })
    ...
    try:
        ...
        col1.write("********************************************")
        col1.subheader("Pica8 Cyber Kill Switch Portal IS READY!")
        col1.write("********************************************")
    ...
    return username, password, loggedin, pica8apikey, instantlockbutton, backupthelabbutton, goldenlabbackupbutton

def qlockdown(col2expander, username, password, loggedin, pica8apikey, tokenheaders, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, datetimestamp, sftp, deviceslist):
    col2expander.write ("Quarantining the Network Now.....Please Standby.....")
    switchListing = {}
    configListing = {}
    if (deviceschosen):
        sgselector = []
        for device in deviceschosen:
            col2expander.write ("Backing up Config on....." + device + " Before Proceeding.......Please Standby.....")
            jobspayload=""
    ...
    if (sgselector):
        ...
        for switchgroupname in sgselector:
            print ("Backing up each device in " + switchgroupname + " Before Proceeding....Standby....")
            col2expander.write ("Backing up each device in " + switchgroupname + " Before Proceeding....Standby....")
            response = requests.request("GET", groupswitchlisturl, headers=tokenheaders, data=emptypayload, verify=False)
            switchconfigsresponse = response.text
            print (switchconfigsresponse)
    ...
    for playbookjobstatus in playbookjobsstatus:
        ...
        if ( (str(playbookjobstatus['name'])) == playbookjob):    
            ...
            col2expander.write ('*********************************\n')
            col1.subheader("NETWORK BACKUP COMPLETE!")
    if (deviceschosen):
        sgselector = []
        for device in deviceschosen:
            payload = json.dumps({
                "config_name": "quarantineport",
                "switch_checked": {
                    "checkall": allchecked,
                    "checkedNodes": [
                    {
            ...
            col2expander.write ("Locking Down....." + device + ".......Please Standby.....")
            #jobspayload=""
            configpushurl = pica8apiurl + "/config_files/push"
            print (configpushurl)
            response = requests.request("POST", configpushurl, headers=tokenheaders, data=payload, verify=False)
            switchconfigsresponse = response.text
            print (switchconfigsresponse)
            col2expander.write ( device + " LOCKDOWN COMPLETE! KILLSWITCH ACTIVATED!")
            col2expander.write ('*********************************\n')
            col1.subheader(device +  " LOCKDOWN COMPLETE! **KILLSWITCH ACTIVATED!")
    if (sgselector):
            print ("SG SELECTED!")
            print("**************************")
            print (sgselector)
            ...
            sgdevices = []
            for switchgroupname in sgselector:
                print ("Locking Down ALL devices in " + switchgroupname + "....Standby....")
                col2expander.write ("Locking Down ALL devices in " + switchgroupname + "....Standby....")
                # response = requests.request("GET", groupswitchlisturl, headers=tokenheaders, data=emptypayload, verify=False)
                # switchconfigsresponse = response.text
                # print (switchconfigsresponse)
                # switchgroupslist = json.loads(switchconfigsresponse)
                ...
                switchconfigsresponse = response.text
                print (switchconfigsresponse)
                col2expander.write ( switchgroupname + " LOCKDOWN COMPLETE! KILLSWITCH ACTIVATED!")
                col2expander.write ('*********************************\n')
                col1.subheader(switchgroupname + " LOCKDOWN COMPLETE! KILLSWITCH ACTIVATED!")
    if (allchecked == True):
        deviceschosen = []
        sgselector = []
        alldevices = []
        print ("Locking Down ***ALL*** devices ....Standby....")
        col2expander.write ("Locking Down ***ALL*** devices ....Standby....")
                # response = requests.request("GET", groupswitchlisturl, headers=tokenheaders, data=emptypayload, verify=False)
                # switchconfigsresponse = response.text
                # print (switchconfigsresponse)
                # switchgroupslist = json.loads(switchconfigsresponse)
        ...
        switchconfigsresponse = response.text
        print (switchconfigsresponse)
        col2expander.write ("TOTAL NETWORK LOCKDOWN COMPLETE! KILLSWITCH ACTIVATED!")
        col2expander.write ('*********************************\n')
        col1.subheader("TOTAL NETWORK LOCKDOWN COMPLETE! KILLSWITCH ACTIVATED!")            
    
    
def instantlock(col2expander, username, password, loggedin, pica8apikey, tokenheaders, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, datetimestamp, sftp, deviceslist):
    col2expander.write ("Locking Down ALL Switches.....Please Standby.....")
    switchListing = {}
    configListing = {}
    print ("Backup headers: " + str(tokenheaders))
    print ("The key: " + str(pica8apikey))
...
def disengagelock(col2expander, username, password, loggedin, pica8apikey, tokenheaders, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, customrestore_tag, dateselected, datetimestamp, sftp, deviceslist):
    col2expander.write ("Disengaging KillSwitch and Resetting Switches to Backup Config Tag " + customrestore_tag + ".....Please Standby.....")
    ...
    if (deviceschosen):
        ...
        for device in deviceschosen:
            payload = json.dumps({
                "config_name": "disengageportquarantine",
                "switch_checked": {
                    "checkall": allchecked,
                    "checkedNodes": [
                    {
            ...
            col2expander.write ("Disengaging Killswitch on....." + device + ".......Please Standby.....")
            #jobspayload=""
            configpushurl = pica8apiurl + "/config_files/push"
            print (configpushurl)
            response = requests.request("POST", configpushurl, headers=tokenheaders, data=payload, verify=False)
            switchconfigsresponse = response.text
            print (switchconfigsresponse)
            col2expander.write ( device + " UNLOCK COMPLETE! KILLSWITCH DE-ACTIVATED!")
            col2expander.write ('*********************************\n')
            col1.subheader(device +  " UNLOCK COMPLETE! \n\n **KILLSWITCH DE-ACTIVATED!")
    if (sgselector):
            print ("SG SELECTED!")
            print("**************************")
            print (sgselector)
            ...
            device = ""
            for switchgroupname in sgselector:
                print ("Un-Locking ALL devices in " + switchgroupname + "....Standby....")
                col2expander.write ("Un-Locking ALL devices in " + switchgroupname + "....Standby....")
                # response = requests.request("GET", groupswitchlisturl, headers=tokenheaders, data=emptypayload, verify=False)
                # switchconfigsresponse = response.text
                # print (switchconfigsresponse)
                # switchgroupslist = json.loads(switchconfigsresponse)
                ...
                switchconfigsresponse = response.text
                print (switchconfigsresponse)
                col2expander.write ( switchgroupname + " UNLOCK COMPLETE! KILLSWITCH DE-ACTIVATED!")
                col2expander.write ('*********************************\n')
                col1.subheader(switchgroupname + " UNLOCK COMPLETE! \n\n KILLSWITCH DE-ACTIVATED!")
    if (allchecked == True):
        deviceschosen = []
        sgselector = []
        alldevices = []
        device = ""
        print ("UN-Locking ***ALL*** devices ....Standby....")
        col2expander.write ("UN-Locking ***ALL*** devices ....Standby....")
                # response = requests.request("GET", groupswitchlisturl, headers=tokenheaders, data=emptypayload, verify=False)
                # switchconfigsresponse = response.text
                # print (switchconfigsresponse)
                # switchgroupslist = json.loads(switchconfigsresponse)
        ...
        switchconfigsresponse = response.text
        print (switchconfigsresponse)
        col2expander.write ("TOTAL NETWORK LOCKDOWN DISENGAGED! KILLSWITCH DE-ACTIVATED!")
        col2expander.write ('*********************************\n')
        col1.subheader("TOTAL NETWORK LOCKDOWN DISENGAGED! \n\nKILLSWITCH DE-ACTIVATED!")            
...
def goldenlabbackup(col2expander, username, password, loggedin, pica8apikey, tokenheaders, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, datetimestamp, sftp, deviceslist):
...
username, password, loggedin, pica8apikey, instantlockbutton, backupthelabbutton, goldenlabbackupbutton  = LOGIN(username, password, loggedin, pica8apikey, instantlockbutton, backupthelabbutton, goldenlabbackupbutton)
print (loggedin)
...
if (loggedin == "success"):
    ...
    response = requests.request("GET", switchlisturl, headers=tokenheaders, data=emptypayload, verify=False)
    switchconfigsresponse = response.text
    print (switchconfigsresponse)
    ...
    col1.markdown(f"# STEP 1: Select a Group of Devices, or Individual Devices To QUARANTINE or LOCKDOWN")
    ...
    alldevices = col1.checkbox("Select This Option to QUARANTINE OR LOCKDOWN ALL Devices in AmpCon", disabled=False, key="1")
    ...
    dvselector = alldeviceschosen.multiselect("Select an Individual Device or Select Devices",devicenames, key="dvselector", disabled=False)
    ...
    if alldevices:
        ...
        alldeviceschosen.multiselect("Select an Individual Device or Select Devices",devicenames, key="dvselector1", disabled=True)
        allchecked = True 
    ...
    col2.markdown(f"# STEP 2: SECURE THE NETWORK! LOCKDOWN/RESTORE ALL DEVICES")   
    col2.subheader(f"Config Backups of all devices being locked down *WILL* be done to preserve current Network Topology.\n\n")
    col2.write("*********************************\n\n")
    col2.write("*********************************\n\n")
    col2.subheader(f"**MANDATORY:**\n\n ENTER A CUSTOM TAG BELOW TO BE USED WITH BACKUP (and RESTORE) OF CONFIGS AFTER NETWORK LOCKDOWN OR QUARANTINE IS DISENGAGED (AND ADDED TO BACKUP CONFIG FILENAME) FOR CUSTOM RESTORE")
    custombackup_tag = col2.text_input('ENTER CUSTOM TAG:')
    col2.subheader("**NOTE: SAVED FILENAMES**\n\n Files will automatically have the hostname and date followed by your custom tag followed by the format-style of the file (I.e. SET commands format, or native Pica8 XML restore format).\n\n Files will be saved both on device (/home/admin/<filename>) and on AmpCon (/home/admin/Config_Backups/<tagname>/) for future load/save):\n\n I.e. EVPN-ACC2_01-01-2022_MyTag_SET/RESTORE.conf")
    col2.write("********************************************")
    col2.write("********************************************")
    col2.subheader("**ONLY IF DIS-ENGAGING LOCKDOWN/QUARANTINE**: \n\n")
    col2.write("********************************************")
    col2.markdown(f"SELECT THE DATE WHEN THE LAB CONFIG BACKUP **WAS MADE**:")
    dateselected = col2.date_input("SELECT DATE WHEN BACKUP **WAS MADE**:")
    col2.markdown(f"ENTER THE CUSTOM TAG **USED TO MAKE THE PREVIOUS BACKUP OF THE LAB**. THIS CONFIGURATION WILL BE RESTORED TO ALL SELECTED DEVICES.")
    customrestore_tag = col2.text_input('ENTER THE CUSTOM TAG FROM THE PREVIOUS BACKUP.')
    col2.write("*********************************\n\n")
    col2.subheader("**NOTE: LOCKDOWN OR QUARANTINE**\n\n  Network Device Configs will be backed up automatically before the device is secured, if ***no*** custom \"backup\" tag is provided **ABOVE** in the custom BACKUP tag field, a custom backup tag will be created automatically in the form of: current_lab_config+current date/time and added to the filename.\n\n")
    col1.write("********************************************")
    col1.markdown(f"# STEP 3: PRESS A BUTTON :)")
    qlockdownbutton = col1.button('QUARANTINE OR LOCKDOWN NETWORK DEVICE OR SEGMENT EXCEPT FOR MANAGEMENT ACCESS ONLY!')
    #goldenlabbackupbutton = col1.button('MAKE THE LAB\'s CURRENT CONFIG \'THE GOLDEN CONFIG\'')
    instantlockbutton = col1.button('INSTANTLY LOCKDOWN THE ENTIRE NETWORK EXCEPT FOR MANAGEMENT ACCESS')
    disengagelockbutton = col1.button('DISENGAGE LOCKDOWN/QUARANTINE')  
    backup_date = str(dateselected)
    ...
    if ((qlockdownbutton and custombackup_tag) and (sgselector or dvselector or alldevices)):
        print("************************************")
        print("BACKING UP CURRENT CONFIGRUATIONS AND SECUREING THE DEVICES!!!!!!!")
        print("************************************")
        with st.spinner("Secureing the Devices from the network and themselves! "):
                        col2expander = col1.expander("Lockdown Progress Viewer (Press the '+' to Monitor Progress) ")
                        with col2expander:
                            qlockdown(col2expander, username, password, loggedin, pica8apikey, headers, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, datetimestamp, sftp, deviceslist)
    ...
    #                     with col2expander:
    #                         goldenlabbackup(col2expander, username, password, loggedin, pica8apikey, headers, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, datetimestamp, sftp, deviceslist)
                
    if instantlockbutton:
        print("************************************")
        print("CYBER KILL SWITCH ACTIVATED! QUARANTINING ALL NETWORK DEVICES TO CONTAIN THREAT!!!!!!!")
        print("************************************")
        with st.spinner("Secureing ALL Devices from the network and themselves!"):
                        col2expander = col1.expander("Kill Switch Progress Viewer (Press the '+' to Monitor Progress) ")
                        with col2expander:
                            instantlock(col2expander, username, password, loggedin, pica8apikey, headers, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, datetimestamp, sftp, deviceslist)
                
    if (( disengagelockbutton and customrestore_tag) and (sgselector or dvselector or alldevices)):
        print("************************************")
        print("DISENGAGING LOCKDOWN AND RESTORING NETWORK ACCESS with Config " + str(customrestore_tag) + " CONFIGS!!!!!!!")
        print("************************************")
        with st.spinner("DISENGAGING LOCKDOWN AND RESTORING NETWORK ACCESS with Config " + str(customrestore_tag) + " Config Standby..."):
                        col2expander = col1.expander("Network Restore Progress Viewer (Press the '+' to Monitor Progress) ")
                        with col2expander:
                            disengagelock(col2expander, username, password, loggedin, pica8apikey, headers, pica8apiurl, sgselector, allchecked, deviceschosen, switchgroup_devices, custombackup_tag, customrestore_tag, dateselected, datetimestamp, sftp, deviceslist)
